...
`rllm-analyze` 只负责常规训练结果分析和简单调参建议。当出现以下情况时，不要继续堆叠 learning_rate、temperature、num_generations、batch、epoch 等纯参数建议，必须自动调用独立 skill 进入深层诊断：

```text
Skill("rllm-analyze-deep", args="<run_id>")
```

调用 `Skill("rllm-analyze-deep")` 后，当轮响应必须立即结束，等待 deep skill 注入后再继续执行诊断；不得只提示用户手动运行。

升级条件：

| 现象 | 说明 | 动作 |
|---|---|---|
| 连续 2 轮调参后 reward / accuracy 仍无明显改善 | 问题可能不在超参数 | 建议进入 `/rllm-analyze-deep` |
...
输出要求：触发升级条件时，调参建议中只保留必要的安全建议，并明确写出“当前问题不像单纯超参数问题，建议使用 `/rllm-analyze-deep <run_id>` 做深层诊断”。
