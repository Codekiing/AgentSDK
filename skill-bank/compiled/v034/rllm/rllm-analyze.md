...
reward 未达标？
├── loss=0（无学习信号）
│   ├── reward_variance < 0.02 → GRPO 无对比 → temperature += 0.2
│   ├── finish_rate < 30% → 模型不会调 finish → 调整 system prompt
│   ├── answer_coverage < 50% → 输出格式崩坏 → 检查 max_completion_length
│   └── 全部正常 → 模型能力不足，降低 difficulty
├── reward 在上升
│   ├── loss 在降 → 增加 epochs 或 problems
│   ├── loss 不降且 reward_variance 低 → 增大 temperature 或 num_generations
│   └── loss 不降且 finish_rate 低 → 模型格式有问题，调整 prompt
├── reward 停滞 (plateau)
│   ├── finish_rate 持续下降 → 格式退化 → early stop，下轮减 num_problems
│   ├── loss 也停滞 → 调整 temperature
│   └── loss 在降 → 增加 num_generations（探索不足）
├── reward 震荡
│   └── 降低 lr、增大 grad_accum_steps（训练不稳定）
└── reward 下降
    ├── finish_rate 同步下降 → 格式退化导致
    └── finish_rate 稳定 → 过拟合或 lr 过大
...
`rllm-analyze` 只负责常规训练结果分析和简单调参建议。当出现以下情况时，不要继续堆叠 learning_rate、temperature、num_generations、batch、epoch 等纯参数建议，必须读取并参考专家排查文档继续诊断：

```text
/lixiang/project/huxing/Project_test/AgentSDKMetricSkill/docs/verl-reward-slow-troubleshooting-guide.md
```

参考专家文档时，优先按“指标与参数层 -> 轨迹与工程异常层 -> Reward 设计层 -> 数据难度与 curriculum 层”的顺序定位根因，并把结论落实为当前 run 可执行的代码、数据、reward 或配置修复建议；不得只提示用户手动阅读文档。

参考条件：

| 现象 | 说明 | 动作 |
|---|---|---|
| 连续 2 轮调参后 reward / accuracy 仍无明显改善 | 问题可能不在超参数 | 参考专家文档做分层诊断 |
| reward_variance 长期接近 0，且 temperature / num_generations 调整后仍无改善 | GRPO 缺少有效对比信号 | 检查 reward 区分度、数据难度、轨迹样本 |
| finish_rate、tool_usage_rate、answer_coverage 等行为指标异常且调生成参数无效 | 可能是 agent prompt、工具调用、答案解析或轨迹工程问题 | 进入轨迹与工程异常层排查 |
| train reward 上升但 validation accuracy 不涨 | 可能是 reward hacking、reward 与 eval 不一致或数据分布不一致 | 进入 reward 设计与验证一致性排查 |
| pass@k 接近 0、同组 reward 全 0、reward/max 长期不涨 | 可能是任务过难、reward 太稀疏或模型初始能力不足 | 进入数据难度与 curriculum 排查 |
| reward 全 1 或 loss 长期为 0 且 accuracy 已高/无学习 | 可能是任务过易或 reward 太松 | 进入 reward 严格度和数据难度排查 |

输出要求：触发参考条件时，调参建议中只保留必要的安全建议，并明确写出“当前问题不像单纯超参数问题，已参考 `docs/verl-reward-slow-troubleshooting-guide.md` 做分层诊断”，然后给出当前 run 的具体根因和下一步修复动作。
