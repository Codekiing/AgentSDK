...
不再在 Monitor 工具中内联 Python 脚本。统一使用 `rllm_train/monitor_agent.py`：

**流式监控 (stream 模式)**:
```bash
python -m rllm_train.monitor_agent stream rllm_train/output/runs/<run_id>/training_log.txt --timeout 3600
```
...
**兜底轮询 (oneshot 模式)**:
```bash
python -m rllm_train.monitor_agent oneshot rllm_train/output/runs/<run_id>/training_log.txt --last 3
```
