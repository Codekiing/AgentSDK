...
当 config.json 中包含 `"backend": "verl"` 时，监控 `verl_metrics.jsonl`（VERL file logger 输出的结构化 JSON，每步一行）。

**核心原则**: 使用 `tail + python3 -c` 内联模式，避免调用外部 Python 脚本文件（会被 auto mode 阻止）。
...
for line in sys.stdin:
    try:
        ...
        if s > last:
            last = s
            data = d['data']
            r = data.get('critic/score/mean', '-')
            l = data.get('actor/pg_loss', 0)
            g = data.get('actor/grad_norm', 0)
            e = data.get('actor/entropy', 0)
            cl = data.get('response_length/mean', 0)
            print(f'Step {s} | R {float(r):.4f} | Loss {float(l):+.4f} | Grad {float(g):.4f} | Ent {float(e):.3f} | Len {float(cl):.0f}')
    except: pass
"
```
...
CronCreate 每 3 分钟执行，使用 Bash 工具内联 python：

```bash
tail -3 rllm_train/output/runs/<run_id>/verl_metrics.jsonl 2>/dev/null | python3 -c "
import sys, json
lines = [l.strip() for l in sys.stdin if l.strip()]
if not lines:
    print('ONESHOT: no metrics yet (model loading)')
    sys.exit(0)
for line in lines[-3:]:
    d = json.loads(line)
    s = d['step']
    ...
    e = data.get('actor/entropy', 0)
    g = data.get('actor/grad_norm', 0)
    sc_max = data.get('critic/score/max', 0)
    print(f'Step {s} | R {r:.4f} | Ent {e:.3f} | Grad {g:.4f} | max={sc_max:.4f}')
    # Alert conditions
    if e > 2.0:
        print(f'ALERT: entropy > 2.0 at step {s} — possible collapse!')
    if r == 0 and len(lines) >= 2:
        prev = json.loads(lines[-2])
        if prev['data'].get('critic/score/mean', 1) == 0:
            print(f'ALERT: 2 consecutive zero-reward steps!')
"
```
...
```bash
wc -l < rllm_train/output/runs/<run_id>/verl_metrics.jsonl
```

或扫描 training_log.txt 尾部：

```bash
grep -c "Training completed\|Training finished" rllm_train/output/runs/<run_id>/training_log.txt 2>/dev/null
```

#### VERL 指标格式

verl_metrics.jsonl 每行包含完整 step 指标（13+ 字段）：

```
Step N | R 0.xxx | Loss 0.xxx | Grad 0.xxx | Ent 0.xxx | Len xxx
---
critic/score/mean=0.xxx | critic/score/max=1.000 | critic/score/min=0.000
critic/rewards/mean=0.xxx | actor/pg_loss=0.xxx | actor/entropy=0.xxx
actor/grad_norm=0.xxx | actor/pg_clipfrac=0.000 | actor/ppo_kl=0.000
actor/lr=0.000 | response_length/mean=xxx | response_length/clip_ratio=0.000
perf/mfu/actor_infer=0.xxx
```
...
| 指标 | jsonl 字段 | 正常范围 |
|------|-----------|---------|
| 平均奖励 | `critic/score/mean` | > 0.3 |
| 最高奖励 | `critic/score/max` | > 0.5 |
| 策略损失 | `actor/pg_loss` | -0.1 ~ 0.1 |
| 梯度范数 | `actor/grad_norm` | 0.01 ~ 1.0 |
| 探索熵 | `actor/entropy` | 0.05 ~ 0.50 (警报: > 2.0) |
| PPO clip | `actor/pg_clipfrac` | 0 ~ 0.3 |
| KL 散度 | `actor/ppo_kl` | 接近 0 |
| 回复长度 | `response_length/mean` | < max_response_length |

#### VERL 异常检测

| 异常 | 检测方式 | 处理 |
|---|---|---|
| Entropy 爆炸 | entropy > 2.0 (来自 jsonl) | CIRCUIT_BREAK: fix_preset=entropy_bonus |
| 连续零 reward | score/mean=0 连续 3+ 步 | CIRCUIT_BREAK: fix_preset=lr_half |
| Grad 爆炸 | grad_norm > 100 | CIRCUIT_BREAK: fix_preset=lr_tenth |
| OOM | "CUDA out of memory" (training_log.txt) | CIRCUIT_BREAK: fix_preset=lr_half_grad_clip |
| 进程崩溃 | Traceback / RuntimeError (training_log.txt) | 报告错误详情 |
| 训练停滞 | >10min 无新 jsonl 写入 | 报告警告 |
