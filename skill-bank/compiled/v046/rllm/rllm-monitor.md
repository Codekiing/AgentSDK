...
当 config.json 中包含 `"backend": "verl"` 时，使用 VERL 专用监控代理。

#### 流式监控

```bash
python rllm_train/monitor_agent_verl.py stream rllm_train/output/runs/<run_id> --timeout 3600
```

#### 兜底轮询

```bash
python rllm_train/monitor_agent_verl.py oneshot rllm_train/output/runs/<run_id> --last 3
```

#### VERL 指标格式

从 `verl_metrics.jsonl`（file logger）读取结构化 JSON，每步完整指标包括：

```
Step N | critic/score/mean=0.xxx | critic/score/max=... | actor/pg_loss=... | actor/entropy=... | actor/grad_norm=... | actor/pg_clipfrac=... | actor/ppo_kl=... | actor/lr=... | response_length/mean=... | response_length/clip_ratio=... | perf/mfu/actor_infer=...
```
...
| 指标 | 含义 | 正常范围 |
|------|------|---------|
| `critic/score/mean` | 平均奖励 | > 0.3 |
| `critic/score/max` | 最高奖励 | > 0.5 |
| `actor/pg_loss` | 策略损失 | -0.1 ~ 0.1 |
| `actor/grad_norm` | 梯度范数 | < 100 |
| `actor/entropy` | 探索熵 | 0.05 ~ 0.5 |
| `actor/pg_clipfrac` | PPO clip 比例 | 0 ~ 0.3 |
| `response_length/mean` | 平均回复长度 | < max_response_length |

#### VERL 训练完成标志

1. `verl_metrics.jsonl` 步数达到预期（total_epochs * num_problems / train_batch_size）
2. 训练进程正常退出 (exit code 0)
3. 日志中出现 `Training completed` 或 `Training finished`

#### VERL 异常检测

通过扫描 `training_log.txt` 尾部 5KB 检测：

| 异常 | 检测方式 | 处理 |
|---|---|---|
| Loss 爆炸 | grad_norm > 100 或 NaN (来自 jsonl) | CIRCUIT_BREAK: fix_preset=lr_tenth |
| Reward 归零 | score/mean 连续 3+ step = 0 (来自 jsonl) | 报告警告 |
| OOM | "CUDA out of memory" | CIRCUIT_BREAK: fix_preset=lr_half_grad_clip |
| 进程崩溃 | Traceback / RuntimeError / Killed | 报告错误详情 |
| Ray Actor 死亡 | "RayActorError" / "Actor died" | 报告错误详情 |
| 训练停滞 | >10min 无新的 jsonl 写入 | 报告警告 |
| 策略崩溃 | score/mean 从 >0 骤降至 0 且不恢复 | CIRCUIT_BREAK: fix_preset=entropy_bonus |
