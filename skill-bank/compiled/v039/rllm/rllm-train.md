...
### Phase 5.5: 深层诊断升级 (rllm-analyze-deep)

rllm-analyze 完成后，编排者读取 analysis.json，根据以下条件决定是否升级到 rllm-analyze-deep：

#### 升级触发条件（任一满足即触发）

| 条件 | 检测方式 | 含义 |
|------|---------|------|
| D1 — 调参无法解决 | analysis.json 中 `suggestions` 为空，或 `primary_action` = `diagnose`/`engineering_fix` | 问题不在超参数 |
| D2 — 连续 plateau | 连续 2 轮 `reward.reached` = false 且 `reward.trend` = `plateau` | 调参已无边际收益 |
| D3 — 行为指标持续异常 | analysis.json 中 `behavior.*.trend` 多项为 `falling` 且调参后无改善 | 可能是工程/数据/reward 设计问题 |
| D4 — 熔断后完整分析仍无改善 | circuit_break_count >= 2 后走完整 Phase 5，但 analysis.json 显示 reward 仍不达标且无有效调参建议 | 快速路径和完整分析都失败 |

#### 升级后的流程

```
Phase 5 (rllm-analyze) → analysis.json
  ↓
编排者读取 analysis.json
  ↓
检查 D1-D4
  ├── 未触发 → 正常回到 Phase 2 (调参)
  └── 触发 → Phase 5.5 (rllm-analyze-deep)
                ↓
              rllm-analyze-deep 全自动执行:
                1. 读取 analysis.json + training_log.txt + trajectories
                2. 按 15 层诊断体系逐层排查
                3. 定位根因层次 (指标/轨迹/reward/数据)
                4. 输出 deep_analysis.json
                ↓
              rllm-analyze-deep 完成后
                ├── 诊断结论: 超参数可修 → 回到 Phase 2，传入 deep_analysis.json 的调参建议
                ├── 诊断结论: 需工程修复 → 输出修复方案，暂停等用户确认后继续
                └── 诊断结论: 数据/reward 设计问题 → 输出修改方案，暂停等用户确认后继续
```

#### 编排者执行步骤

1. **读取 analysis.json**，检查 D1-D4:
   ```bash
   python -c "
   import json
   a = json.load(open('rllm_train/output/runs/<run_id>/analysis.json'))
   suggestions = a.get('suggestions', [])
   reached = a.get('reward', {}).get('reached', False)
   trend = a.get('reward', {}).get('trend', '')
   has_tunable = any(s.get('param') for s in suggestions)
   print(json.dumps({
       'reached': reached,
       'trend': trend,
       'has_tunable_suggestions': has_tunable,
...
2. **如果 D1-D4 任一触发**:
   ...
   - rllm-analyze-deep 会自动读取所有数据并做分层诊断
   - 等待 rllm-analyze-deep 完成

3. **rllm-analyze-deep 完成后**:
   - 读取 `deep_analysis.json`
   - 如果 `action = "tune"`: 回到 Phase 2，传入调参建议
   - 如果 `action = "engineering_fix"`: 输出修复方案和代码变更清单，暂停等用户确认
   - 如果 `action = "data_or_reward_fix"`: 输出修改方案，暂停等用户确认

#### 禁止行为

- rllm-analyze 输出 diagnose 后不升级，继续用 diagnose 去调 rllm-config
- rllm-analyze-deep 诊断到工程问题后跳过用户确认直接改代码
- 连续触发 D1-D4 超过 3 次仍不回推到用户做高层次决策
